...
#![cfg(all(feature = "malachite-bigint", not(any(Py_LIMITED_API))))]
//!  Conversions to and from [malachite-bigint](https://docs.rs/malachite-bigint)’s [`BigInt`] and [`BigUint`] types.
//!
...
#![cfg_attr(docsrs, cfg_attr(docsrs, doc = concat!("pyo3 = { version = \"", env!("CARGO_PKG_VERSION"),  "\", features = [\"malachite-bigint\"] }")))]
#![cfg_attr(
    not(docsrs),
    doc = "pyo3 = { version = \"*\", features = [\"malachite-bigint\"] }"
)]
//! ```
//!
//! Note that you must use compatible versions of malachite-bigint and PyO3.
//! The required malachite-bigint version may vary based on the version of PyO3.
//!
//! ## Examples
//!
...
macro_rules! bigint_conversion {
    ($rust_ty: ty, $is_signed: expr, $to_bytes: path, $from_bytes: path) => {
        #[cfg_attr(docsrs, doc(cfg(feature = "malachite-bigint")))]
